...
def run_multi_OR_random(num_attributes, iterations = 100):
    ...
    for index in range(iterations):
        ...
        for i in range(num_attributes):
            ...
            success_flag, rt, rt_with_setup, entry = or_property(index, random_indices[i], count, message, m_prime, m_att, vals[i], other_vals[i], A1, A2, q1, q2, N, k_hat, t11, t21, B, [disclose_indices[i]], r1, bound, setup_start,protocol_start, num_attributes,G)
            count = count + 1
...
def run_OR_false(iterations=100):
    ...
    for index in range(iterations):
        ...
        count = 1
        success_flag, rt, rt_with_setup, entry = or_property(index, random_indices[0], count, message, m_prime, m_att, val, other_val, A1, A2, q1, q2, N, k_hat, t11, t21, B, disclose_indices, r1, bound, setup_start,protocol_start, 1,G)
...
def disclose_false(iterations=100):
    ...
    for index in range(iterations):
        ...
        val = random_vals[1]
        message = np.random.choice(2, (1,1,N))
        # print(int(disclose_indices),int(disclose_indices[0]), disclose_indices)
    ...
    for index in range(iterations):    
        ...
        for att in range(num_attributes):
            ...
            for bit in range(len(disclose_indices[att])):
                ...
                success_flag, rt, rt_with_setup, entry = or_property(index, random_indices[bit], count, message, m_prime, m_att, val, other_val, A1, A2, q1, q2, N, k_hat, t11, t21, B, [disclose_indices[att][bit]], r1, bound, setup_start,protocol_start, num_attributes, G)
                if success_flag == False:
                    break
            if success_flag == False:
                break
