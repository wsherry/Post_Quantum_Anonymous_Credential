...
def run_multi_NOT(num_attributes, iterations = 100):
    ...
    for index in range(iterations):
        ...
        while rejection_abort and h_coeffs_zero:  
            ...
            if rejection_sampling_1(z, poly_matmul_no_mod(np.array(challenge),ring_transpose(np.array(r1)), poly_mod, N), xi) == 0:
                ...
                data.append({"Index": index, "Time":rt, "Time (with Setup)":rt_with_setup ,"Rej Count": rejection_fail_count, "H Coeffs Rerun": h_coeffs_zero_count, "Number Attributes": num_attributes,"Attribute Value":vals, "Not Attribute Value": not_vals, "Attribute Index":disclose_indices , "Success":status, "Error": error, "message": message, "m_prime":m_prime, "m_att": m_att})
                run_times.append(rt)
    ...
    return data, avg_not, med_not, min_not, max_not
    ...
    for index in range(iterations):
        ...
        while rejection_abort:  
            ...
            if rejection_sampling_1(z, poly_matmul_no_mod(np.array(challenge),ring_transpose(np.array(r1)), poly_mod, N), xi) == 0:
                ...
                rt_with_setup = time_value - setup_start
                rt = time_value - protocol_start
                data.append({"Index": index, "Time":rt, "Time (with Setup)":rt_with_setup , "Rej Count": rejection_fail_count, "Attribute Value":val, "Not Attribute Value": not_val, "Attribute Index":disclose_indices , "Success":status, "Error": error, "message": message, "m_prime":m_prime, "m_att": m_att})
                run_times.append(rt)
                run_times_setup.append(rt_with_setup)   
...
def run_multi_OR_random(num_attributes, iterations = 100):
    ...
    for index in range(iterations):
        ...
        for i in range(num_attributes):
            ...
            prover_sizes, verifier_sizes, success_flag, rt, rt_with_setup, entry = or_property_size_info(p_size, v_size, prover_sizes, verifier_sizes,index, random_indices[i], count, message, m_prime, m_att, vals[i], other_vals[i], A1, A2, q1, q2, N, k_hat, t11, t21, B, [disclose_indices[i]], r1, bound, setup_start,protocol_start, num_attributes,G)
            count = count + 1
            ...
            data.append({"Index": index, "Time":rt, "Time (with Setup)":rt_with_setup ,"Rej Count": 0, "Number Attributes": num_attributes,"Attribute Value":vals, "Not Attribute Value":other_vals, "Attribute Index":disclose_indices, "Success":True, "Error": -1, "message": message, "m_prime_1":m_prime_1, "m_prime_2": m_prime_2, "m_att_1": m_att_1, "m_att_2":m_att_2})
...
def run_OR_false(iterations=100):
    ...
    for index in range(iterations):
        ...
        count = 1
        prover_sizes, verifier_sizes, success_flag, rt, rt_with_setup, entry = or_property_size_info(p_size, v_size, prover_sizes, verifier_sizes,index, random_indices[0], count, message, m_prime, m_att, val, other_val, A1, A2, q1, q2, N, k_hat, t11, t21, B, disclose_indices, r1, bound, setup_start,protocol_start, 1,G)
    ...
    return data, avg, med, min, max, #avg_setup, med_setup, min_setup, max_setup
    ...
    for index in range(iterations):
        ...
        while rejection_abort:  
            ...
            if rejection_sampling_1(z, poly_matmul_no_mod(np.array(challenge),ring_transpose(np.array(r1)), poly_mod, N), xi) == 0:
                ...
                rt_with_setup = time_value - setup_start
                rt = time_value - protocol_start
                data.append({"Index": index, "Time":rt,  "Time(with Setup)": rt_with_setup, "Rej Count": rejection_fail_count, "Number Attributes": num_attributes, "Attribute Value":vals, "Attribute Index":disclose_indices, "Success":status, "Error": error, "message": message, "m_prime":m_prime, "m_att": m_att})
                run_times.append(rt)            
                run_times_setup.append(rt_with_setup)
    ...
    for index in range(iterations):
        ...
        while rejection_abort:
            ...
            if rejection_sampling_1(z, poly_matmul_no_mod(np.array(challenge),ring_transpose(np.array(r1)), poly_mod, N), xi) == 0:
                ...
                rt_with_setup = time_value - setup_start
                rt = time_value - protocol_start
                data.append({"Index": index, "Time":rt, "Time(with Setup)": rt_with_setup, "Rej Count": rejection_fail_count,"Attribute Value":val,"Attribute Index":disclose_index, "Success":status, "Error": error, "message": message, "m_prime":m_prime, "m_att": m_att})
                run_times.append(rt)
                run_times_setup.append(rt_with_setup)        
    ...
    for index in range(iterations):  
        ...
        for att in range(num_attributes):
            ...
            for bit in range(len(disclose_indices[att])):
                ...
                prover_sizes_tmp, verifier_sizes_tmp, success_flag, rt, rt_with_setup, entry = or_property_size_info(p_size, v_size, [],[],index, random_indices[bit], count, message, m_prime, m_att, val, other_val, A1, A2, q1, q2, N, k_hat, t11, t21, B, [disclose_indices[att][bit]], r1, bound, setup_start,protocol_start, num_attributes, G)
                if success_flag == False:
                    break
        ...
        if success_flag == True and num_NOT_attributes > 0:
            ...
            while rejection_abort and h_coeffs_zero:  
                ...
                prover_sizes.append(p_size + int(prover_sizes_tmp[0]))
                verifier_sizes.append(v_size+ int(verifier_sizes_tmp[0]))
                if rejection_sampling_1(z, poly_matmul_no_mod(np.array(challenge),ring_transpose(np.array(r1)), poly_mod, N), xi) == 0:
                    rejection_fail_count += 1
                else:
                    rejection_abort = False
...
###################################################################
field_names_NOT = ['Index', 'Time', 'Time (with Setup)', 'Rej Count' ,"H Coeffs Rerun", 'Number Attributes', 'Attribute Value', 'Not Attribute Value', 'Attribute Index','Success', 'Error', "message", "m_prime", "m_att"]

for i in range(1,6):
    data, avg, med, min, max = run_multi_NOT(i,num_iterations)
...
# ####################################################################
data, avg, med, min, max = run_NOT_false(num_iterations)
csv_file = foldername +'//false_NOT.csv'
field_names_NOT = ['Index', 'Time', 'Time (with Setup)','Rej Count' , 'Attribute Value', 'Not Attribute Value', 'Attribute Index','Success', 'Error', "message", "m_prime", "m_att"]
write_file(csv_file,field_names_NOT, data)

# ##################################################################
# # RUNNING MULTI OR TESTS
# ##################################################################
field_names_OR = ['Index', 'Time','Time (with Setup)' ,'Rej Count' ,'Number Attributes','Attribute Value', 'Not Attribute Value', 'Attribute Index','Success', 'Error', "message", "m_prime_1", "m_prime_2", "m_att_1", "m_att_2"]

for i in range(1,6):
    data, avg, med, min, max = run_multi_OR_random(i,num_iterations)
...
####################################################################
csv_file = foldername +'//False_OR.csv'
data, avg, med, min, max = run_OR_false(num_iterations)
field_names_OR = ["Index","Time","Time (with Setup)","Actual Value", 'Rej Count' ,'Number Attributes',"Attribute Value", "Not Attribute Value", "Attribute Index", "Success", "Error", "message", "m_prime_1", "m_prime_2", "m_att_1", "m_att_2"]
write_file(csv_file,field_names_OR, data)
...
# ###################################################################
field_names_disclose = ['Index', 'Time','Time(with Setup)', 'Rej Count' ,'Number Attributes','Attribute Value','Attribute Index','Success', 'Error', "message", "m_prime", "m_att"]

for i in range(1,6):
    data, avg, med, min, max = run_multi_disclose(i,num_iterations)
    csv_file = foldername + '//multi_DISCLOSE_' + str(i) +'_att.csv'
    write_file(csv_file,field_names_disclose, data) 

####################################################################
# RUNNING DISCLOSE TESTS (with False values)
####################################################################
data, avg, med, min, max = disclose_false(num_iterations)
csv_file =  foldername +'//False_DISCLOSE.csv'
field_names_disclose = ["Index", "Time", "Time(with Setup)", 'Rej Count' , "Attribute Value", "Attribute Index", "Success", "Error", "message", "m_prime", "m_att"]
write_file(csv_file,field_names_disclose, data)

####################################################################
# RUNNING MULTI RANGE TESTS
####################################################################
field_names_range = ['Index', 'Time', 'Time (with Setup)','Rej Count' , 'Number Attributes', 'Attribute Value', 'Not Attribute Value', 'Attribute Index','Success', 'Error', "message", "m_prime_1", "m_prime_2", "m_att_1", "m_att_2"]

for i in range(1,6):
    data, avg, med, min, max = run_multi_range(i,iterations=num_iterations)
    csv_file = foldername + '//multi_RANGE_' + str(i) +'_att.csv'
