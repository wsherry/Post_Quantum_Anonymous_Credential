...
d = 512         # degree of polynomial ring
...
def rej1(z, v, std, M):
    ...
    # print("zv + vnorm",RF(sg.exp(((-2) * dot_Rql(z, v) + (v_norm**2))), 2))
    # print("std", RF(2 * (std**2)))
    return  RF(u) > RF(expr)
...
def abdlop_single_quadratic(m1, m2, ell, n,
    ...
    w = A1 * y1 + A2 * y2
    # print("-------y1, -B,y2:",len(y1), B.nrows(), B.ncols(),len(y2))
    ...
    # print(s.dot_product(R2 * y))
    # print( y.dot_product(R2 * s))
    # print(r1.dot_product(y))
    ...
    if rej_u_1(z1, cs1, std1, rep_M1):
        print(691, "----------------------------------single quad rej1")
        return "Rejected"

    cs2 = c * s2
    z2 = cs2 + y2
    if rej_u_2(z2, cs2, std2, rep_M2):
        print(697,"-----------------------------------single quad rej2")
        return "Rejected"
    ...
    if (cond1 and cond2 and cond3) == False: 
        print(711,"-----------------------------------single quad fail", cond1, cond2, cond3)
        temp = z.dot_product(R2 * z) + c * r1.dot_product(z) + c**2 * r0 - f
        # q2 = 67108837
        # print(temp)
        print()
        # print(temp.list(), temp)
        print(temp.list())
        # print(Rql_2_ZZl(temp)%q2)
        print("________________")
        print(v.list())
        # print(sg.matrix(sg.Integers(q2), temp), sg.matrix(sg.Integers(q2), v))

    # print("running time single quad: ", time.perf_counter()- start)
    global data
    data = data + ["single quad", str(time.perf_counter()- start)]
    print("single quadratics result ========= ",cond1 and cond2 and cond3)
...
# def R_to_S_mat(S,mat, n,m):
#     new_mat = sg.matrix(S, n,m)
#     for i in range(n):
#         new_row = sg.vector(S, n)
#         for j in range(m):
#             coeffs = Rql_2_ZZl(mat[i][j])
#             # print(coeffs)
#             poly_q2 = S(sum(c * t**k for k, c in enumerate(coeffs)))
#             new_row[j] = poly_q2
#         new_mat[i][j] = new_row
...
def abdlop_multiple_quadratic(m1, m2, ell, n, N,
    ...
    # =============================== Verifier ================================
    mu_is = rand_Rql(N)
    # print("-------944, mu_is, mu_is[0], R2_is[0]:",N,R2_is[0].nrows(),R2_is[0].ncols())
...
def abdlop_quadratic_poly(m1, m2, ell, n, k, N, M, lambd,
    ...
    if not is_opti:
        ...
        # print("R2_is",R2_is.nrows(), R2_is.ncols())
        r1_is.extend([stack_vec_Rql([
    ...
    cond1 = abdlop_multiple_quadratic(m1, m2, ell, n, N,
                                      ...
                                      R2_is, r1_is, r0_is)
    if cond1 == "Rejected":
        # print(1177, "----------------------------------quad poly fail cond1 rejected")
        return "Rejected"
    
    if cond1 == False:
        print(1191, "----------------------------------quad poly fail cond1 fail")
    #=============================== Verifier ================================
    cond2 = all(h[i][0] == 0 for i in range(lambd))  # check constant coefficient is 0
    # print("quadratic polynomial cond 2", cond2)
    if cond2 == False :
        print(1184, "----------------------------------quad poly fail cond2 fail")
...
def abdlop_mlwe(m1, m2, n, k, Z, n_is, lambd,
    ...
    """
    global d, IR, P, t, R, X, IR_q, P_q, t_q, R_q, X_q
    d = 512
    IR = sg.IntegerRing()
    P = sg.PolynomialRing(IR, 't')
    t = P.gen()
    ...
    y3_smplr = GaussianSampler(R_q, std3, 1)
    y3 = y3_smplr.get()
    ...
    #=============================== Verifier ================================
    R_ = rand_ZZ_mat_binomial(new_val, n_ex *d, 2)
    # print(sg.matrix(R_q,s3).nrows(),sg.matrix(R_q,s3).ncols())
    #================================ Prover =================================
    # print(len(y3), R_.nrows(), R_.ncols(), len(s3))
    ...
    e_is = [sg.vector(R_q, [R_q([1 if k + d*j == i else 0 for k in range(d)]) for j in range(new_val//d)])
            for i in range(256)]

    K_s = sg.block_matrix(R_q, [
        [IM(m1), ZM(m1, Z), ZM(m1, new_val//d + 1)]
    ]) * J

    k_theta_is = [stack_vec_Rql([zv(m1 + i), [1], zv(Z - i - 1 + new_val//d + 1)]) * J for i in range(Z)]

    K_y3 = sg.block_matrix(R_q, [
        [ZM(1, m1 + Z), IM(1), ZM(1, 1)]
    ]) * J

    k_beta3 = (ZM(1, m1 + Z + 1).augment(sg.matrix(R_q, [1])) * J)[0]
    ...
    R2_p_is.extend([ZM(2*(m1 + Z + 1 + 1), 2*(m1 + Z + 1 + 1)) for i in range(1, d)])
    ...
    cond1 = abdlop_quadratic_poly(m1, m2, ell, n, k, N, M, lambd,
                                  ...
                                  R2_p_is, r1_p_is, r0_p_is)
    if cond1 == "Rejected":
        # print(1465, "----------------------------------mlwe fail cond1")
        return "Rejected"

    #=============================== Verifier ================================
    cond2 = (float(norm_Rql(z3)) <= float(roh * std3 * sg.sqrt(256)))
    if cond2 == False:
        print(1471, "----------------------------------mlwe fail cond2")
...
def abdlop_toolbox(m1, m2, ell, n, k, N, M, n_bin, Z, n_is, n_p, lambd,
    ...
    z3 = y3 + beta3 * Rs3
    print("checking here")
    print(type(Rs3), type(z3), type(y3), type(beta3))
    # exit()
    print("REJ 3")
    if rej_u_3(z3, Rs3, std3, rep_M3):
        print(1562, "--------------rej 3")
    ...
    append_result(data)
    print("toolbox result conditions", cond2, cond3)
    return (cond1 and cond2 and cond3)
